...
- 인스턴스를 시작할 때 또는 인스턴스가 실행 중이거나 중지 된 후에 인스턴스에 대한 세부 모니터링을 활성화 할 수 있습니다. 그렇지 않으면 기본 모니터링이 활성화됩니다. 기존 인스턴스에 대한 세부 모니터링을 활성화하는 구문
...
- **Fargate**
  ...
  **Fargate의 Amazon Elastic Container Service (Amazon ECS)-Amazon** Elastic Container Service (Amazon ECS)는 클러스터에서 Docker 컨테이너를 쉽게 실행, 중지 및 관리 할 수있는 확장 성이 뛰어나고 빠른 컨테이너 관리 서비스입니다. Fargate 시작 유형을 사용하여 서비스 또는 작업을 시작하여 Amazon ECS에서 관리하는 서버리스 인프라에서 클러스터를 호스팅 할 수 있습니다.
  ...
  AWS Fargate는 Amazon Elastic Container Service (ECS) 및 Amazon Elastic Kubernetes Service (EKS) 모두에서 작동하는 컨테이너 용 서버리스 컴퓨팅 엔진입니다. Fargate를 사용하면 애플리케이션 구축에 쉽게 집중할 수 있습니다. Fargate를 사용하면 서버를 프로비저닝하고 관리 할 필요가 없으며 애플리케이션 당 리소스를 지정하고 비용을 지불 할 수 있으며 설계에 따른 애플리케이션 격리를 통해 보안을 개선 할 수 있습니다.
...
개발자 어소시에이트로서 AWS Lambda가 처리 된 후 DLQ에 메시지를 추가하는 사용 사례를 식별하도록 도와 줄 수 있습니까?
...
- **IAM 역할 사용**
  ...
  애플리케이션이 사용하는 보안 자격 증명을 관리 할 필요없이 애플리케이션이 안전하게 인스턴스에서 API 요청을 할 수 있도록 IAM 역할이 통합되었습니다. AWS 자격 증명을 생성하고 배포하는 대신 IAM 역할을 사용하여 API 요청을 할 수있는 권한을 위임 할 수 있습니다.

  Amazon EC2는 인스턴스 프로필을 IAM 역할의 컨테이너로 사용합니다. IAM 콘솔을 사용하여 IAM 역할을 생성하면 콘솔이 인스턴스 프로파일을 자동으로 생성하고 해당하는 역할과 동일한 이름을 부여합니다.

  EC2에 할당 된 역할을 사용하여 EC2 인스턴스에 자격 증명을 저장하지 않고도 S3에 액세스 할 수 있으므로 가장 안전한 옵션입니다.
...
- **Amazon S3 데이터 일관성 모델**
  - Amazon S3는 현재 동시 업데이트에 대한 객체 잠금을 지원하지 않습니다. 객체 잠금은 S3 객체 잠금 기능과 다릅니다. S3 Object Lock을 사용하면 WORM (Write-Once-Read-Many) 모델을 사용하여 객체를 저장하고 고정 된 시간 동안 또는 무기한으로 객체를 삭제하거나 덮어 쓰는 것을 방지 할 수 있습니다.
  - **프로세스는 Amazon S3에 새 객체를 작성하고 즉시 버킷 내의 키를 나열합니다. 변경 사항이 완전히 전파 될 때까지 오브젝트가 목록에 표시되지 않을 수 있습니다** . 이는 유효한 명령문입니다.
  - **프로세스는 기존 개체를 삭제하고 즉시 읽기를 시도합니다. 삭제가 완전히 전파 될 때까지 Amazon S3는 삭제 된 데이터를 반환 할 수 있습니다** . 이는 유효한 설명입니다.
  - **프로세스는 기존 객체를 삭제하고 즉시 버킷 내의 키를 나열합니다. 삭제가 완전히 전파 될 때까지 Amazon S3는 삭제 된 객체를 나열 할 수 있습니다** . 이는 유효한 설명입니다.

- **버킷 A 오리진이 요청을 수행 할 수 있도록 JavaScript 파일을 호스팅하는 버킷 B에 CORS를 구성합니다.**

  CORS (Cross-Origin Resource Sharing)는 한 도메인에로드 된 클라이언트 웹 애플리케이션이 다른 도메인의 리소스와 상호 작용하는 방법을 정의합니다.

  교차 출처 요청을 허용하도록 버킷을 구성하려면 버킷 액세스를 허용 할 출처, 각 출처에 대해 지원할 작업 (HTTP 메서드)을 식별하는 규칙이있는 XML 문서 인 CORS 구성을 생성합니다. 및 기타 작업 관련 정보.

  주어진 사용 사례 `<CORSRule>`에 `<CORSConfiguration>`대해 버킷 B 에 대한 in 을 생성하여 버킷 A에서 호스팅되는 S3 웹 사이트 오리진에서 액세스를 허용합니다.
...
- **LSI 생성**
  - LSI는 Local Secondary Index를 나타냅니다. 일부 애플리케이션은 기본 테이블의 기본 키를 사용하여 데이터를 쿼리하기 만하면됩니다. 그러나 대체 정렬 키가 도움이되는 상황이있을 수 있습니다. 애플리케이션에 정렬 키를 선택하려면 테이블에 하나 이상의 local secondary index를 생성하고 이러한 인덱스에 대해 Query 또는 Scan 요청을 발행 할 수 있습니다.
  - LSI (Local Seconday Index)
    - 해시 키에 로컬인 테이블의 대체 범위 키
    - LSI는 반드시 정의되어야 함

- **GSI 만들기** 
  - GSI (Global Secondary Index)는 파티션 키와 기본 테이블의 정렬 키와 다를 수있는 정렬 키가있는 인덱스입니다.
  - GSI (Global Secondary Index)
    - 키가 아닌 속성에 대한 쿼리 속도를 높이려면 GSI
    - 파티션 키 + 옵션 정렬 키
    - **추가/수정 가능 (LSI는 불가능)**
    - GSI에서(만) 쓰기가 쓰로틀링 되면 기본 테이블이 조절 됨
...
**Amazon EBS는 AWS KMS와 연동**하여 EBS 볼륨을 암호화 및 해독합니다. EC2 인스턴스의 부팅 및 데이터 볼륨을 모두 암호화 할 수 있습니다. 암호화 된 EBS 볼륨을 생성하고 지원되는 인스턴스 유형에 연결하면 다음 유형의 데이터가 암호화됩니다.

1. 볼륨 내부에 저장된 데이터
2. 볼륨과 인스턴스간에 이동하는 모든 데이터
3. 볼륨에서 생성 된 모든 스냅 샷
4. 해당 스냅 샷에서 생성 된 모든 볼륨

- **EBS 볼륨** 은 **KMS를 사용하여 전송 중 암호화와 미사용 암호화를 모두 지원합니다** . 이는 올바른 설명입니다. 암호화 작업은 EC2 인스턴스를 호스팅하는 서버에서 발생하여 인스턴스와 연결된 EBS 스토리지간에 미사용 데이터와 전송중인 데이터의 보안을 모두 보장합니다.
...
Amazon ElastiCache는 Amazon RDS와 같은 데이터 스토어를위한 이상적인 프런트 엔드로, 요청 속도가 매우 높거나 지연 시간이 짧은 애플리케이션에 고성능 미들 티어를 제공합니다. 캐싱의 가장 중요한 부분은 구현이 최소한으로 침습적이며 그렇게함으로써 규모와 속도 모두에 관한 애플리케이션 성능이 크게 향상된다는 것입니다.
...
**클러스터 모드에서 Amazon ElastiCache Redis 구현** -클러스터 모드가 활성화 된 Redis 용 ElastiCache를 활용하여 기존 워크로드를 거의 변경하지 않고도 안정성과 가용성을 향상시킬 수 있습니다. 클러스터 모드는 Redis 클러스터의 수평 적 확장의 주요 이점과 함께 제공되며 클러스터 성능에 거의 영향을주지 않습니다.

프로덕션 워크로드를 빌드 할 때 데이터를 쉽게 다시 작성할 수없는 경우 복제와 함께 구성 사용을 고려해야합니다. 클러스터 모드를 활성화하면 클러스터를 확장하는 데 여러 가지 추가 이점이 있습니다. 간단히 말해, 노드 유형을 확장 또는 축소 (수직 확장)하는 대신 샤드 수를 확장하거나 축소 할 수 있습니다 (수평 확장). 즉, Cluster-Mode는 최대 90 개의 샤드에 걸쳐 매우 많은 양의 스토리지 (잠재적으로 100 테라 바이트)로 확장 할 수있는 반면 단일 노드는 인스턴스 유형에 용량이있는만큼만 메모리에 데이터를 저장할 수 있습니다.

**Amazon ElastiCache Memcached 구현** -Redis 및 Memcached는 인기있는 오픈 소스 인 메모리 데이터 스토어입니다. 둘 다 사용하기 쉽고 고성능을 제공하지만 엔진을 선택할 때 고려해야 할 중요한 차이점이 있습니다. Memcached는 단순성을 위해 설계되었으며 Redis는 광범위한 사용 사례에 효과적으로 사용할 수있는 다양한 기능을 제공합니다. Redis는 스냅 샷 기능, 복제를 제공하고 트랜잭션을 지원하므로 Memcached는 불가능하므로 ElastiCache Redis는 사용 사례에 적합한 선택입니다.
...
#### CloudFormation을 사용하여 리소스를 프로비저닝하는 서비스는?

**AWS Elastic Beanstalk** -AWS Elastic Beanstalk는 Apache와 같은 친숙한 서버에서 Java, .NET, PHP, Node.js, Python, Ruby, Go 및 Docker로 개발 된 웹 애플리케이션 및 서비스를 배포하고 확장하기위한 사용하기 쉬운 서비스입니다. , Nginx, Passenger 및 IIS. Elastic Beanstalk는 AWS CloudFormation을 사용하여 환경에서 리소스를 시작하고 구성 변경 사항을 전파합니다.

**AWS 서버리스 애플리케이션 모델 (AWS SAM)** -AWS SAM 사양을 사용하여 서버리스 애플리케이션을 정의합니다. AWS SAM 템플릿은 AWS CloudFormation 템플릿의 확장이며 작업을 더 쉽게 해주는 몇 가지 추가 구성 요소가 있습니다. AWS SAM은 구성의 기반으로 CloudFormation 템플릿이 필요합니다.

잘못된 옵션 :

**AWS Lambda** -AWS Lambda를 사용하면 서버를 프로비저닝하거나 관리하지 않고도 코드를 실행할 수 있습니다. 사용한 컴퓨팅 시간에 대해서만 비용을 지불합니다. 따라서 Lamda는 서비스를 실행하기 위해 CloudFormation이 필요하지 않습니다.

**AWS Autoscaling** -AWS Auto Scaling은 애플리케이션을 모니터링하고 용량을 자동으로 조정하여 가능한 가장 낮은 비용으로 안정적이고 예측 가능한 성능을 유지합니다. AWS Auto Scaling을 사용하면 몇 분 만에 여러 서비스에서 여러 리소스에 대한 애플리케이션 확장을 쉽게 설정할 수 있습니다. Auto Scaling은 CloudFormation을 사용했지만 필수 요구 사항은 아닙니다.

**AWS CodeBuild** -AWS CodeBuild는 소스 코드를 컴파일하고 테스트를 실행하며 배포 할 준비가 된 소프트웨어 패키지를 생성하는 완전 관리 형 지속적 통합 서비스입니다. CodeBuild를 사용하면 자체 빌드 서버를 프로비저닝, 관리 및 확장 할 필요가 없습니다. AWS CodePipeline은 AWS CloudFormation을 배포 작업으로 사용하지만 필수 서비스는 아닙니다.
...
- **CORS (** Cross-Origin Resource Sharing)는 한 도메인에로드 된 클라이언트 웹 응용 프로그램이 다른 도메인의 리소스와 상호 작용하는 방법을 정의합니다. **API의 리소스가 API의 자체 도메인이 아닌 다른 도메인에서 요청을 수신하고 이러한 요청의 서비스를 제한하려는 경우 리소스에서 선택한 메서드에 대해 CORS (교차 출처 리소스 공유)를 비활성화해야합니다.**
  다른 승인되지 않은 도메인이 API에 액세스하는 것을 방지하려고할 때 사용
  
- **CORS 활성화**
  
  CORS (Cross-Origin Resource Sharing)는 한 도메인에로드 된 클라이언트 웹 애플리케이션이 다른 도메인의 리소스와 상호 작용하는 방법을 정의합니다. CORS 지원을 통해 Amazon S3로 풍부한 클라이언트 측 웹 애플리케이션을 구축하고 Amazon S3 리소스에 대한 교차 출처 액세스를 선택적으로 허용 할 수 있습니다.
...
AWS Organizations에서 조직을 생성 한 경우 해당 조직의 모든 AWS 계정에 대한 모든 이벤트를 기록하는 추적을 생성 할 수도 있습니다. 이를 조직 추적이라고합니다.

**기본적으로 CloudTrail은 버킷 수준 작업 만 추적합니다. 객체 수준 작업을 추적하려면 Amazon S3 데이터 이벤트를 활성화해야** 합니다. 이는 올바른 설명입니다. AWS CloudTrail은 버킷 이벤트와 별도로 Amazon S3 데이터 이벤트를 지원합니다. S3 객체에 대한 모든 API 작업을 기록하고 호출자의 AWS 계정, 호출자의 IAM 사용자 역할, API 호출 시간, API의 IP 주소 및 기타 세부 정보와 같은 세부 정보를받을 수 있습니다. 모든 이벤트는 S3 버킷 및 CloudWatch 이벤트로 전달되므로 이벤트에 대한 프로그래밍 작업을 수행 할 수 있습니다.

**멤버 계정은 조직 추적을 볼 수 있지만 수정하거나 삭제할 수는 없습니다.** 조직 추적은 마스터 계정에서 생성되어야하며 조직에 적용하도록 지정되면 조직의 모든 멤버 계정에 자동으로 적용됩니다. 회원 계정은 조직 추적을 볼 수 있지만 수정하거나 삭제할 수 없습니다. 기본적으로 멤버 계정은 Amazon S3 버킷의 조직 추적에 대한 로그 파일에 액세스 할 수 없습니다.

회원 계정은 조직 추적을 볼 수 있지만 수정하거나 삭제할 수 없습니다. 기본적으로 멤버 계정은 Amazon S3 버킷의 조직 추적에 대한 로그 파일에 액세스 할 수 없습니다.
...
- **CloudWatch 이벤트 규칙 사용**

  Amazon CloudWatch Events는 AWS 리소스와 AWS에서 실행하는 애플리케이션을 모니터링하는 웹 서비스입니다. Amazon CloudWatch Events를 사용하여 파이프 라인, 단계 또는 작업 상태의 변화를 감지하고 이에 대응할 수 있습니다. 그런 다음 생성 한 규칙에 따라 CloudWatch Events는 파이프 라인, 단계 또는 작업이 규칙에서 지정한 상태가 될 때 하나 이상의 대상 작업을 호출합니다. Amazon CloudWatch Events 규칙 및 대상의 예 :

  1. 인스턴스 상태가 변경 될 때 알림을 보내는 규칙입니다. 여기서 EC2 인스턴스는 이벤트 소스이고 Amazon SNS는 이벤트 대상입니다.
  2. CodeBuild 구성이 이벤트 소스이고 Amazon SNS가 이벤트 대상인 빌드 단계가 변경 될 때 알림을 보내는 규칙입니다.
  3. 파이프 라인 변경을 감지하고 AWS Lambda 함수를 호출하는 규칙입니다.

---

**Write Through 전략 사용**

write-through 전략은 데이터가 데이터베이스에 기록 될 때마다 캐시의 데이터를 추가하거나 업데이트합니다.

Write Through 전략에서 새 블로그 또는 블로그 업데이트는 데이터베이스 계층과 캐싱 계층 모두에 기록되므로 항상 캐시에서 최신 데이터가 제공됩니다.

---

**CodeStar**

AWS CodeStar를 사용하면 AWS에서 애플리케이션을 신속하게 개발, 구축 및 배포 할 수 있습니다. AWS CodeStar는 통합 된 사용자 인터페이스를 제공하므로 한 곳에서 소프트웨어 개발 활동을 쉽게 관리 할 수 있습니다. AWS CodeStar를 사용하면 몇 분 만에 전체 지속적 배포 도구 체인을 설정할 수 있으므로 코드 릴리스를 더 빨리 시작할 수 있습니다. AWS CodeStar를 사용하면 전체 팀이 안전하게 공동 작업 할 수 있으므로 액세스를 쉽게 관리하고 프로젝트에 소유자, 기여자 및 뷰어를 추가 할 수 있습니다. 각 AWS CodeStar 프로젝트에는 Atlassian JIRA Software에서 제공하는 통합 문제 추적 기능을 포함한 프로젝트 관리 대시 보드가 함께 제공됩니다. AWS CodeStar 프로젝트 대시 보드를 사용하면 작업 항목 백 로그에서 팀의 최근 코드 배포에 이르기까지 전체 소프트웨어 개발 프로세스의 진행 상황을 쉽게 추적 할 수 있습니다.

---

**TTL과 함께 지연로드 전략 사용**

지연로드는 필요한 경우에만 데이터를 캐시에로드하는 캐싱 전략입니다. 애플리케이션이 데이터를 요청할 때마다 먼저 ElastiCache 캐시를 요청합니다. 데이터가 캐시에 있고 최신 데이터 인 경우 ElastiCache는 데이터를 애플리케이션에 반환합니다. 데이터가 캐시에 없거나 만료 된 경우 애플리케이션은 데이터 저장소에서 데이터를 요청합니다. 그러면 데이터 저장소가 데이터를 애플리케이션에 반환합니다.

이 경우 사용자가 적극적으로 요청한 데이터는 ElastiCache에 캐시되며 TTL 덕분에 데이터 부실을 제한하기 위해 1 분 후에 해당 데이터를 만료시킬 수 있습니다.

---

**DHW**

AWS Certificate Manager는 AWS 서비스 및 내부 연결 리소스와 함께 사용할 퍼블릭 및 프라이빗 SSL / TLS (Secure Sockets Layer / Transport Layer Security) 인증서를 쉽게 프로비저닝, 관리 및 배포 할 수있는 서비스입니다. SSL / TLS 인증서는 네트워크 통신을 보호하고 인터넷을 통한 웹 사이트의 ID와 사설 네트워크의 리소스를 설정하는 데 사용됩니다. AWS Certificate Manager는 SSL / TLS 인증서를 구매, 업로드 및 갱신하는 시간 소모적 인 수동 프로세스를 제거합니다.